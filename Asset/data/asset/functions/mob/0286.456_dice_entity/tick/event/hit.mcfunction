...
# ダメージ処理
    ...
    # 属性をランダムに決定する
        ...
        # 火
            ...
            execute if score @s Temporary matches 0 at @e[tag=LandingTarget,limit=1] run particle dust 1000000000 1 0 1 ~ ~1 ~ 0.3 0.5 0.3 0.01 20 force
        # 水
    ...
    # ダメージ
        execute at @a if score @p UserID = @s CU.UserID as @p run function api:damage/modifier
