...
# オーバーワールドか確認
    ...
    execute if entity @s[tag=CanUsed] unless predicate lib:dimension/is_overworld run tag @s remove CanUsed
# 今が夜かどうか確認
    execute if entity @s[tag=CanUsed] if predicate lib:is_day run tellraw @s {"text":"発動条件を満たしていません。","color":"red"}
    execute if entity @s[tag=CanUsed] if predicate lib:is_day run tag @s remove CanUsed
